...
bool is_same_bytes(const InContainer1 &bytes1, const InContainer2 &bytes2) {
  return is_same_bytes(bytes1.begin(), bytes1.end(), bytes2.begin(),
                       bytes2.end());
}

template <typename InContainer1, typename InContainer2>
void check_equal_bytes(const InContainer1 &bytes1, const InContainer2 &bytes2) {
  EXPECT_TRUE(is_same_bytes(bytes1, bytes2));
}
...
template <typename OutIter>
void hex_string_to_bytes(const std::string &hex_str, OutIter first,
                         OutIter last) {
  EXPECT_GE(hex_str.length(), 2 * std::distance(first, last));
...
const std::pair<std::string, std::string> sample_message_list[sample_size] = {
    ...
    mess_and_hash(
        "This is exactly 64 bytes long, not counting the terminatin",
        "af38fc14dbbbcc6cd4c9cc73988e1b08373b4e6b04ba61b41f999731185b51af"),
    mess_and_hash(
        "This is exactly 64 bytes long, not counting the terminating",
        "f778b361f650cdd9981ca13adb77f26b8419a407b3938fc54b14e9971045fa9d"),
    mess_and_hash(
        "This is exactly 64 bytes long, not counting the terminating b",
        "9aa72d139c7d7e5a35ea525e2ba6704163555ba647927765a61ccbf12ec60479")};

void test_sha(size_t i) {
  ...
  }
  {
    ...
    EXPECT_EQ(ans_hex_str, hash_hex_str);
  }
  {
    std::string hash_hex_str;
  ...
  {
    std::vector<unsigned char> hash(picosha2::k_digest_size);
    picosha2::hash256(src_vect.begin(), src_vect.end(), hash.begin(),
                      hash.end());
    check_equal_bytes(ans, hash);
  }
  {
  ...
  }
  {
    ...
    EXPECT_EQ(one_by_one_hex_string, hex_string);
  }
  {
    std::string one_by_one_hex_string;
    {
...
TEST(Picosha2Test, TestCase1) { test_sha(0); }

TEST(Picosha2Test, TestCase2) { test_sha(1); }

TEST(Picosha2Test, TestCase3) { test_sha(2); }

TEST(Picosha2Test, TestCase4) { test_sha(3); }

TEST(Picosha2Test, TestCase5) { test_sha(4); }

TEST(Picosha2Test, TestCase6) { test_sha(5); }

TEST(Picosha2Test, TestCase7) { test_sha(6); }

TEST(Picosha2Test, TestCase8) { test_sha(7); }

TEST(Picosha2Test, TestCase9) { test_sha(8); }

TEST(Picosha2Test, TestCase10) { test_sha(9); }
